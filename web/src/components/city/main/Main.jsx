...
import { getArticlesAPI,getArticlesAPI2, updateArticleAPI } from "../../../action";
import PostalModal from "../postalmodal/PostalModal";
...
function Main(props) {
	...
	function likeHandler(event, postIndex, id) {
		event.preventDefault();
		let currentLikes = 3;//props.articles[postIndex].likes.count; TODO QND TIVER back
		let whoLiked = [];//props.articles[postIndex].likes.whoLiked; TODO QND TIVER back
		let user = 'email@email.com'//props.user.email; TODO QND TIVER AUTH
	...
	return (
		<Container>
			...
			<Content>
				...
				{/* false aq em baixo, TODO qnd tiver back */}
				...
				{ props.articles.length > 0 && props.articles.map((article, key) => (
					<Article key={key}>
						<SharedActor>
							<a>
								...
								<div>
									...
									<span>{article.actor.date}</span>{/* <span>{article.actor.date.toDate().toLocaleDateString()}</span> */}
								</div>
							</a>
							<button>
						...
						<SocialActions>
							{/* <button onClick={(event) => likeHandler(event, key, props.ids[key])} className={article.likes.whoLiked.indexOf(props.user.email) >= 0 ? "active" : null}> TODO QUANDO TIVER AUTH*/}
							<button onClick={(event) => likeHandler(event, key, props.ids[key])} className={article.likes.whoLiked.indexOf('EMAIL@EMAIL') >= 0 ? "active" : null}>
...
const mapDispatchToProps = (dispatch) => ({
	getArticles: () => dispatch(getArticlesAPI2()),
